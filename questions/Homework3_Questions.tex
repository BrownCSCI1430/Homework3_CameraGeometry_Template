...
Let's calibrate our camera. We want to estimate $\mathbf{M}$ from $N$ known 2D--3D point correspondences $\{(\mathbf{p}_i, \mathbf{P}_i)\}$. Writing $\mathbf{m}_1^\top, \mathbf{m}_2^\top, \mathbf{m}_3^\top$ for the three rows of $\mathbf{M}$ (each a $1\times4$ vector), this gives three scalar equations:
%
\begin{align*}
...
\begin{subquestion}[points=2]
Eliminate the unknown scale $w_i$. Write the resulting equations that are \emph{linear} in the 12 $m_{ij}$ entries of $\mathbf{M}$, and form a constraint by equating them to zero.
\end{subquestion}
